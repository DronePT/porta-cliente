...
const isThemeJSFile = function(file) {
...
gulp.task("scripts", done => gulpSequence("jscripts", "minifyjs")(done));
...
gulp.task("copy-assets", ["copy-images"], function() {
  ...
  // Copy Apex Charts from node_modules
  gulp.src(paths.node + "apexcharts/dist/apexcharts.min.js").pipe(gulp.dest(paths.js));
...
// Deleting all images assets from assets folder
gulp.task("delete-images", function() {
  return del([paths.img + "/**"]);
});

// Run:  gulp copy-images.
// Copy all images assets from src to assets folder
gulp.task("copy-images", ["delete-images"], function copyImages() {
  return gulp.src(paths.srcimg + "/*").pipe(gulp.dest(paths.img + "/"));
});
...
gulp.task("clean-vendor-assets", function() {
  return del([
    ...
    paths.scss + "/bootstrap4/**",
    "./assets/webfonts/fa-**-**.{ttf,woff,woff2,eot,svg}",
    paths.scss + "/fontawesome/**"
  ]);
});
